...
def _post_results(gid, group_type, sender_name, sender_id, original, result_text):
    """Đăng kết quả về group FPT Chat (plaintext), TAG người tạo yêu cầu.
...
def _post_tagged(gid, group_type, sender_name, sender_id, text):
    """Đăng MỘT tin vào FPT group, tag người yêu cầu (mention ở offset 0). Trả inc."""
...
def _process_one(gid, group_type, sender_name, sender_id, text, me):
    ...
    if is_error:
        ...
        return
    # REVIEW: LUÔN báo KẾT QUẢ REVIEW về FPT chat (tag người yêu cầu), bất kể verdict.
    # Việc DUYỆT MERGE là riêng trên Telegram: chỉ khi verdict == APPROVE mới hỏi; chỉ
    # khi bạn ĐỒNG Ý mới báo thêm 'đã merge' về FPT chat, từ chối/hết hạn thì im.
...
def _await_merge(gid, group_type, sender_name, sender_id, mr_iid):
    ...
    báo về FPT chat TRƯỚC đó (ở _process_one), bất kể verdict.

    - BẠN ĐỒNG Ý merge → merge MR + báo thêm 1 tin 'đã merge' về FPT chat (tag người
      yêu cầu).
    - TỪ CHỐI / HẾT HẠN → IM LẶNG phía FPT chat (quyết định merge là việc riêng của bạn).
    ...
    if status != "approved":
        # từ chối / hết hạn → KHÔNG đụng tới FPT chat
        print(f"[{_now()}] [merge] {mr_label}: {status} → không merge, không báo FPT chat.")
        return
    if not mr_iid:
